  ...
  try {
    ...
    if (!fs.existsSync(TEMPLATES_DIR)) {
      ...
      try {
        ...
        console.warn(`Warning: Could not empty ${TEMPLATES_DIR} cleanly, retrying...`);
        try {
            await fs.promises.rm(TEMPLATES_DIR, { recursive: true, force: true, maxRetries: 5 });
            await fs.ensureDir(TEMPLATES_DIR);
        } catch (e2) {
            console.warn(`Warning: Second attempt to empty ${TEMPLATES_DIR} failed:`, e2);
        }
      }
    }
